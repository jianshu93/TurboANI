...
BLAST_TRUTH = BASE / "viral_85_100/blast_truth_af010/teseptimavirus_60_blastani_af010.tsv"
...
def load_truth(path: Path, min_ani: float | None = None) -> dict[tuple[str, str], float]:
    truth: dict[tuple[str, str], float] = {}
    with path.open() as handle:
        reader = csv.DictReader(handle, delimiter="\t")
        for row in reader:
            ani = float(row["blast_ani"])
            af = float(row["af_short"])
            if af < 0.1:
                continue
...
def plot(joined: list[dict[str, object]], out_pdf: Path, axis_min: float, axis_max: float) -> None:
    ...
    for method in ("TurboANI fast", "TurboANI default", "TurboANI slow", "FastANI", "skani"):
        ...
        if not points:
            continue
        ax.scatter(
            [float(row["blastani"]) for row in points],
...
def main() -> None:
    ...
    high_truth = load_truth(BLAST_TRUTH, min_ani=85.0)
    high_joined, high_summary = build_joined(high_truth)
    write_table(OUT / "teseptimavirus60_blast_af010_85_100_vs_methods_joined.tsv", high_joined)
    write_table(OUT / "teseptimavirus60_blast_af010_85_100_vs_methods_summary.tsv", high_summary)
    plot(high_joined, OUT / "teseptimavirus60_blast_af010_85_100_vs_methods_axis85_100.pdf", 85.0, 100.0)

    full_truth = load_truth(BLAST_TRUTH, min_ani=None)
    full_joined, full_summary = build_joined(full_truth)
    write_table(OUT / "teseptimavirus60_blast_af010_vs_methods_joined.tsv", full_joined)
    write_table(OUT / "teseptimavirus60_blast_af010_vs_methods_summary.tsv", full_summary)
    plot(full_joined, OUT / "teseptimavirus60_blast_af010_vs_methods_axis60_100.pdf", 60.0, 100.0)

    print(OUT / "teseptimavirus60_blast_af010_85_100_vs_methods_axis85_100.pdf")
    print(OUT / "teseptimavirus60_blast_af010_vs_methods_axis60_100.pdf")
