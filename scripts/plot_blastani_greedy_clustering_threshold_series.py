...
def load_lengths(path: Path) -> tuple[list[str], dict[str, int]]:
...
def load_blastani(path: Path, genomes: list[str]) -> dict[tuple[str, str], float]:
    raw = pd.read_csv(path, sep="\t")
    raw["genome1_key"] = raw["genome1"].map(genome_key)
    raw["genome2_key"] = raw["genome2"].map(genome_key)
    raw = raw[pd.to_numeric(raw["rbm_ani"], errors="coerce").notna()].copy()

    ani_by_pair: dict[tuple[str, str], float] = {}
    for row in raw.itertuples(index=False):
        g1 = row.genome1_key
        g2 = row.genome2_key
        ani_by_pair[pair_key(g1, g2)] = float(row.rbm_ani)
...
def read_directed_ani(path: Path, genomes: list[str], ani_scale: float) -> dict[tuple[str, str], float]:
    ...
    raw = raw[raw["ani"].notna() & (raw["ani"] >= 0)].copy()
...
def plot_threshold_series(summary: pd.DataFrame, out_path: Path) -> None:
    ...
    legend_handles = [
        Line2D([0], [0], color="black", linestyle="--", marker="x", linewidth=1.25, label="BLASTANI truth")
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Greedy longest-genome representative clustering across ANI thresholds using BLASTANI truth."
    )
    parser.add_argument(
        "--base",
        type=Path,
        default=Path("/Users/jianshuzhao/Library/Mobile Documents/com~apple~CloudDocs/TurboANI_paper/accuracy"),
        help="Directory containing the 300-genome method result files.",
    )
    parser.add_argument(
        "--truth",
        type=Path,
        default=Path("/Users/jianshuzhao/blastANI_300.all_results.tsv"),
        help="BLASTANI truth table.",
    )
    parser.add_argument(
        "--lengths",
        type=Path,
        default=Path("/Users/jianshuzhao/Library/Mobile Documents/com~apple~CloudDocs/TurboANI_paper/accuracy/orthoani_all_44850_pairs.results.tsv"),
        help="Table containing genome lengths for longest-genome representative selection.",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("/Users/jianshuzhao/Documents/Codex/2026-07-23/for/outputs/strep300_blastani_clustering"),
    ...
    genomes, lengths = load_lengths(args.lengths)
    truth_ani = load_blastani(args.truth, genomes)

    method_inputs = [
        ("FastANI", args.base / "strep_300_fastANI_local_t16.txt", 1.0),
        ("TurboANI sensitive 77.5", args.base / "strep_300_turboani_min77_5.txt", 1.0),
        ("TurboANI default 80", args.base / "strep_300_turboani_min80.txt", 1.0),
        ("TurboANI fast 82.5", args.base / "strep_300_turboani_min82_5.txt", 1.0),
        ("skani sensitive", args.base / "strep_300_superani.txt", 100.0),
    ]
    missing = [str(path) for _, path, _ in method_inputs if not path.exists()]
    if missing:
        raise FileNotFoundError("Missing method result files:\n" + "\n".join(missing))
    ...
    out_prefix = args.out_dir / "blastani_greedy_clustering_threshold_series"
